...
public class IOProcessor {
	...
	public IOProcessor() {
		...
		questionManager(new QuestionManager(mUserManager, mDB));
	...
	public void parseNewRequest(HttpServletRequest request, HttpServletResponse response) {
		...
		try {
			...
			if (newRequest != null) {
				 ...
				 * 
				 * if (newRequest.equals("newUser")) addNewUser(request);
				 */
				...
				if (newRequest.equals("assignQuestion")) {
					assignQuestion(request, response);
				}

				if (newRequest.equals("getNextQuestion")) {
					getNextQuestion(request, response);
				}
			}
	...
	/*
	 * // Interprets instructions sent from command line private void
	 * processInstructionString(String prString) { InstructionParser iParser =
	 * new InstructionParser(this);
	 * 
	 * String iInstructions; String iResult = ""; int iLength =
	 * prString.length();
	 * 
	 ...
	 * 
	 * for (int i = 0; i < iLength; i++) { if (iInstructionStringArray[i] !=
	 ...
	 * 
	 * // TODO: Proper parsing. // List all questions if (iInstructions ==
	 * "list questions" || iInstructions == "list q") {
	 * mMessageLogger.newMessage("Questions:", MessageLogger.MESSAGE_SERVER);
	 * 
	 * if (mQuestionManager.questionList().size() > 0) { for (int q = 1; q <=
	 * mQuestionManager.questionList().size(); q++) { iResult +=
	 * mQuestionManager.questionList().get(q).questionString();
	 * mMessageLogger.newMessage(iResult, MessageLogger.MESSAGE_COMMAND);
	 * iResult = ""; } } } // List all users if (iInstructions == "list users"
	 * || iInstructions == "list u") { if (mUserManager.usersOnline().size() >
	 * 0) { mMessageLogger.newMessage("Connected Users:",
	 * MessageLogger.MESSAGE_COMMAND);
	 * 
	 * for (int q = 1; q <= mUserManager.usersOnline().size(); q++) { iResult +=
	 * mUserManager.usersOnline().get(q).userLogin();
	 * mMessageLogger.newMessage(iResult, MessageLogger.MESSAGE_COMMAND);
	 * iResult = ""; } } else { mMessageLogger.newMessage("No users connected",
	 * MessageLogger.MESSAGE_COMMAND); } } }
	 * 
	 * private void processChatString(String prString) { String iChatString =
	 * prString.replace("\0", "");
	 * 
	 * // TODO: Reenable chat messages in JSON ChatMessage iNewMessageReceived =
	 * new ChatMessage();//
	 * mSerialiserMethods.ConvertStringToChatMessage(iChatString);
	 * 
	 ...
	 * 
	 * // New implementation of question input using XML serialisation private
	 * void processNewQuestion(HttpServletRequest request) { Question
	 * iNewQuestion = new Question();//
	 * mSerialiserMethods.ConvertStringToQuestion(prQuestionArray);
	 * 
	 * // Set the question ID if (mQuestionManager.questionList().size() > 0)
	 * iNewQuestion.questionID(mQuestionManager.getLargestQuestionKey() + 1);
	 * else iNewQuestion.questionID(0);
	 * 
	 * // Determine whether the question is being added or modified // TODO:
	 * Implement a better system to do this if
	 * (mQuestionManager.isQuestionStringInUse(iNewQuestion.questionString())) {
	 ...
	 * MessageLogger.MESSAGE_QUESTION);
	 * 
	 * }
	 * 
	 * // TODO: Add JSON parsing for answers private void
	 * processAnswerString(HttpServletRequest request) { char[] iAnswerArray =
	 * new char[150]; // The max char size for all // answers is currently 150
	 * char[] iQuestionIDArray = new char[5]; // allow for up to 99999 ID's
	 * boolean iQuestionIDFound = false; boolean iAnswerFound = false; int
	 * iCurrPos = 0; Answer iNewAnswer = new Answer();
	 * 
	 * if (mAnswerManager.receiveResponses()) {
	 * 
	 * iNewAnswer.username("TEST"); iNewAnswer.answer(new
	 * String(iAnswerArray).replace("\0", ""));
	 * iNewAnswer.questionID(Integer.parseInt(new
	 * String(iQuestionIDArray).replace("\0", "")));
	 * iNewAnswer.answerSent(false);
	 * 
	 * mAnswerManager.addAnswer(iNewAnswer);
	 * 
	 * mMessageLogger.newMessage( "TEST" + " answered " + iNewAnswer.answer() +
	 * " for " +
	 * mQuestionManager.getQuestionStringByID(iNewAnswer.questionID()),
	 * MessageLogger.MESSAGE_ANSWER); } else {
	...
	private User addNewUser(HttpServletRequest request) {
		 ...
		 * mMessageLogger.newMessage(iNewUser.username() + " joined the server",
	...
	private void assignQuestion(HttpServletRequest request, HttpServletResponse response) {
		response.setContentType("text/plain");
		try {
			PrintWriter out = response.getWriter();
	...
	private void getNextQuestion(HttpServletRequest request, HttpServletResponse response) {
		String jsonString = "";
		...
		try {
			...
			jsonString = mQuestionManager.getNextQuestion(username);
